...
import { readVouchers, type UserVoucher } from "../lib/vouchers";
    ...
    <ProtectedPage>
      <OwnerSidebar>
        <section className="p-4 lg:p-8">
          ...
          <p className="body-copy mt-1">{t({ en: "Claimed promotions are saved here and can be used during booking.", zh: "已领取的优惠会保存在这里，预约时可以使用。" })}</p>

          {vouchers.length === 0 ? (
            <div className="villa-card mt-5 text-center">
              <h2 className="card-title">{t({ en: "No vouchers yet", zh: "还没有优惠券" })}</h2>
              <p className="body-copy mt-2">{t({ en: "Claim homepage promotions to fill your voucher wallet.", zh: "到首页领取优惠活动后会显示在这里。" })}</p>
              ...
              {vouchers.map((voucher) => (
                <article key={voucher.id} className="villa-card flex min-h-[190px] flex-col">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className={`rounded-pill px-3 py-1 text-xs font-black ${statusClass(voucher.status)}`}>{voucher.status}</span>
                  </div>
                  ...
                  <div className="mt-auto pt-4 text-xs font-bold text-villa-text-secondary">
                    ...
                    {voucher.usedAt ? <p className="m-0 mt-1">{t({ en: "Used on", zh: "使用日期" })}: {new Date(voucher.usedAt).toLocaleDateString(lang === "zh" ? "zh-CN" : "en-US")}</p> : null}
                    {voucher.orderId ? <p className="m-0 mt-1">Order: {voucher.orderId}</p> : null}
                  </div>
                </article>
              ))}
            </div>
